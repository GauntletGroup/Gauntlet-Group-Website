...
- [x] Add `VITE_CALENDLY_URL` to the local environment variables.
- [x] Create `src/types/calendly.d.ts` defining `Calendly` namespace and options interface.

### Phase 2: Script Loading Hook
- [x] Create `src/hooks/useCalendlyScript.ts` loading `widget.js` and `widget.css`.
- [x] Ensure safe cleanup and check for existing script tags.

### Phase 3: Inline Component Development
- [x] Create `src/components/sections/BookCall.tsx` with a container, a loading skeleton, and the initialization script.
- [x] Configure branding colors (`background_color: '0b1120'`, `text_color: 'ffffff'`, `primary_color: 'f59e0b'`).
- [x] Handle component unmount cleanups and route change states.

### Phase 4: Popup / Modal Integration
- [x] Implement a trigger helper `openCalendlyPopup` using `window.Calendly.showPopupWidget` in `src/hooks/useCalendlyScript.ts`.
- [x] Add state management to `App.tsx` for tracking when/where popup is triggered.

### Phase 5: Navigation & Page Integration
- [x] Add the `<BookCall />` component to the section stack in `App.tsx`.
- [x] Update `Navbar.tsx` to list and scroll to `#book-call`.
- [x] Wire up Hero CTA and Contact Section CTA cards to trigger the booking popup or scroll to the inline embed.
...
- [x] **Chrome (Desktop/Mobile):** Validate responsive scaling and time selector click areas.
- [x] **Safari (Mac/iOS):** Confirm that iframe cookies/cross-site tracking protection (ITP) doesn't block loading of the calendar dates.
- [x] **Firefox:** Ensure scrollbars inside the iframe render cleanly.

### Usability & Performance
- [x] **Layout Shifts (CLS):** Run Lighthouse audit before and after loading the script to verify layout stability.
- [x] **Adblockers:** Verify that standard adblockers (e.g. uBlock Origin) do not block Calendly unless explicit tracking options are enabled. Provide fallback link if blocked.
- [x] **Cookie Banner Compliance:** Confirm that cookies are not set prior to user acceptance if host site consent is active.
- [x] **Timezone Detection:** Verify the widget auto-detects the user's browser timezone correctly.

### Fallback System
- [x] If `useCalendlyScript` fails or script takes > 5 seconds to load, render a user-friendly card: *"Trouble loading the calendar? [Book directly on Calendly](link) or contact us via email."*
