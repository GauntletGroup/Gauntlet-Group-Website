  ...
  return (
    <section id="hero" className="relative min-h-screen flex items-center justify-center overflow-hidden mesh-gradient">
      ...
      >
        ...
        {/* Badge */}
        <motion.div variants={itemVariants} className="inline-flex items-center bg-gray-800/30 backdrop-blur-sm border border-amber-500/30 rounded-full px-6 py-2 mb-8">
          <Zap className="text-amber-400 mr-2" size={16} />
          <span className="text-amber-400 text-sm font-medium uppercase tracking-widest">AI &amp; IT Automation</span>
        </motion.div>
        ...
        <motion.h2 variants={itemVariants} className="text-5xl md:text-7xl lg:text-8xl font-extrabold mb-8 leading-[0.9] tracking-tight">
          AI-Powered IT Automation<br />
          <span className="bg-gradient-to-r from-amber-400 via-blue-500 to-amber-600 bg-clip-text text-transparent">for Teams That Mean Business</span>
        </motion.h2>

        {/* Subheadline */}
        <motion.p variants={itemVariants} className="text-xl md:text-2xl text-gray-400 mb-12 max-w-3xl mx-auto leading-relaxed">
          We design and implement intelligent workflows that triage IT alerts, automate helpdesk tasks, streamline employee onboarding, and connect the tools your business already uses — so your people focus on what matters.
        </motion.p>
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-3 gap-8 mt-20 max-w-2xl mx-auto">
          {[
            ...
            { value: '100%', label: 'Client-Owned Credentials' },
        ...
        <motion.div variants={itemVariants} className="mt-12 flex flex-wrap justify-center gap-3">
          {integrations.map((item, i) => (
            <div
              key={i}
              className="flex items-center gap-2 bg-gray-800/40 border border-white/10 rounded-full px-4 py-2 text-xs font-semibold text-gray-400 hover:border-amber-400/30 hover:text-gray-300 transition-all duration-300"
            >
